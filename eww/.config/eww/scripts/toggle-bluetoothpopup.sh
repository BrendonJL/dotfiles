#!/bin/bash
# Toggle bluetooth popup with cursor-relative positioning
...
if eww active-windows | grep -q 'bluetooth_popup'; then
    eww close bluetooth_popup
else
    # Close any other open popups first
    eww close audio_popup network_popup display_popup battery_popup system_popup calendar_popup workspace_popup power_popup month_picker year_picker event_details event_editor confirm_dialog 2>/dev/null
    ~/.config/eww/scripts/render-bluetoothpopup.sh
    eww open bluetooth_popup --screen "$monitor_info" --pos "${popup_x}x${popup_y}"
fi