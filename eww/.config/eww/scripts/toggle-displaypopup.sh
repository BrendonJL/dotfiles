#!/bin/bash
# Toggle display popup with cursor-relative positioning
...
if eww active-windows | grep -q 'display_popup'; then
    eww close display_popup
else
    # Close any other open popups first
    eww close audio_popup bluetooth_popup network_popup battery_popup system_popup calendar_popup workspace_popup power_popup month_picker year_picker event_details event_editor confirm_dialog 2>/dev/null
    eww open display_popup --screen "$monitor_info" --pos "${popup_x}x${popup_y}"
fi