#!/bin/bash
# Toggle workspace popup with cursor-relative positioning
...
popup_x=$((X - monitor_x_offset - 80))
popup_y=$((Y - monitor_y_offset + -20))

# Keep popup on screen (don't go negative)
[[ $popup_x -lt 0 ]] && popup_x=8
[[ $popup_y -lt 0 ]] && popup_y=1

# Check if already open, close it
if eww active-windows | grep -q 'workspace_popup'; then
    eww close workspace_popup
else
    # Close any other open popups first
    eww close audio_popup bluetooth_popup network_popup display_popup battery_popup system_popup calendar_popup power_popup month_picker year_picker event_details event_editor confirm_dialog 2>/dev/null
    eww open workspace_popup --screen "$monitor_info" --pos "${popup_x}x${popup_y}"
fi